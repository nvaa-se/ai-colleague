...
const createButtonRow = (documentId) => {
  ...
  return new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`edit_${documentId}`).setLabel("Feedback").setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId(`reject_${documentId}`).setLabel("Reject").setStyle(ButtonStyle.Danger)
  );
};
...
\`${scope3}\`
        ${
          parsedJson.reviewComment
            ? `Kommentar från AI-kollegan: ${parsedJson.reviewComment.slice(0, 200)}`
            : ""
        }
        `,
